...
const BoutiqueAdmin = () => {
  ...
  // Charger les packs sauvegardés au démarrage
  useEffect(() => {
    const savedPacksData = localStorage.getItem("savedPacks")
    if (savedPacksData) {
      setSavedPacks(JSON.parse(savedPacksData))
    }
  }, [])

  // Sauvegarder un nouveau pack ou mettre à jour un pack existant
  const savePack = (packData) => {
    if (editingPackIndex !== null) {
      // Mise à jour d'un pack existant
      const updatedPacks = savedPacks.map((p, i) => i === editingPackIndex ? packData : p)
      setSavedPacks(updatedPacks)
      localStorage.setItem("savedPacks", JSON.stringify(updatedPacks))
      setEditingPackIndex(null)
    } else {
      // Ajout d'un nouveau pack
      const updatedPacks = [...savedPacks, packData]
      setSavedPacks(updatedPacks)
      localStorage.setItem("savedPacks", JSON.stringify(updatedPacks))
    }
  }

  // Supprimer un pack sauvegardé
  const removeSavedPack = (index) => {
    const packType = savedPacks[index]?.type;
    const updatedPacks = savedPacks.filter((_, i) => i !== index)
    setSavedPacks(updatedPacks)
    localStorage.setItem("savedPacks", JSON.stringify(updatedPacks))
    // Réinitialiser les quantités du formulaire du pack supprimé
    if (packType) {
      setPackForms((prev) => ({
        ...prev,
        [packType]: Object.fromEntries(Object.keys(prev[packType] || {}).map(key => [key, 0]))
      }))
    }
  }

  // Ouvrir le modal de modification avec pré-remplissage
  const handleEditPack = (idx) => {
    const pack = savedPacks[idx]
    if (!pack) return
    setEditingPackIndex(idx)
    setSelectedPack(customPacks.find(p => p.id === pack.type))
    setPackForms((prev) => ({
      ...prev,
      [pack.type]: { ...pack.services }
    }))
    setShowPackModal(true)
  }

  // Fermer le modal de pack (reset édition)
  ...
  const handleSelectCustomPack = (pack) => {
    if (savedPacks && savedPacks.length > 0) {
      const savedPack = savedPacks.find(s => s.type === pack.id);
      if (savedPack) {
        setOrderToValidate({
          type: "custom",
          pack: pack,
            ...
            <div className="p-8">
              ...
              >
                ...
                <div className="flex justify-between items-center mb-10">
                  ...
                  {/* Bloc Gold et remise supprimé */}
                </div>

                {/* Packs sauvegardés */}
                {savedPacks && savedPacks.length > 0 && savedPacks.map((savedPack, idx) => (
                  <div key={idx} className="max-w-3xl mx-auto bg-white/60 backdrop-blur-lg rounded-3xl shadow-2xl p-8 flex items-center gap-8 relative mt-8">
                    {/* Badge ou icône */}
                    ...
                    <div className="flex-1">
                              ...
                              <>
                                {pack.services.map((service, serviceIndex) => {
                                  const serviceKey = service.name
                                    ...
                                    .replace("reelvideocoure", "reel")
                                    .replace("shootingphotovideo", "shooting");
                                  const quantity = savedPack.services?.[serviceKey] || 0;
                      ...
                      <div className="bg-white/80 rounded-xl p-4 mt-2">
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <span className="text-lg font-bold text-gray-700">{calculatePackTotal(savedPack.type)} DH</span>
                        </div>
                    ...
                    <div className="flex flex-col gap-3">
                      <button onClick={() => removeSavedPack(idx)} className="text-gray-400 hover:text-red-500 transition p-2 self-end" title="Supprimer le pack">
                        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                        </svg>
                      </button>
                      {/* Le bouton Modifier et Prendre ce forfait peuvent être adaptés pour chaque pack */}
                      <button onClick={() => handleEditPack(idx)} className="bg-gray-800 text-white px-6 py-3 rounded-xl font-semibold shadow hover:bg-gray-700 transition">
                        Modifier
                      </button>
                      <button
                ...
                {/* Suppression de la barre de sélection des catégories */}
                ...
                {/* Les cartes des produits standards (Pack Basic, Pro, Premium) sont supprimées ici */}
                {/* <div
                ...
                <div className="space-y-6">
                  <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl p-6 border border-gray-200 sticky top-0">
                    ...
                    <div className="space-y-3 mb-4">
                      ...
                      {/* Suppression de la Remise Gold (-10%) */}
                      {/*
                    ...
                    >
                      <div className="flex items-center justify-center">
                        <span className="text-2xl mr-3">💳</span>
                        Finaliser le paiement ({orderToValidate.total} DH)
                      </div>
                    </motion.button>
                  </div>
                </div>
                    ...
                    <motion.button
                      ...
                      onClick={() => {
                        ...
                        if (editingPackIndex === null) {
                          setPackForms((prev) => ({
                            ...prev,
                            [pack.id]: Object.fromEntries((pack.services || []).map(service => [
                              service.name
                                .toLowerCase()
                                .replace(/[^a-z0-9]/g, "")
                                .replace("photovideo", "")
